...
class Registry():
    ...
    def __init__(self):
        try:
            print('########')
            print(settings.UPSTREAM_DATA_BLOCK)
            print(socket.gethostbyaddr(settings.UPSTREAM_DATA_BLOCK))
            print(socket.gethostbyname(settings.UPSTREAM_DATA_BLOCK))
            print(socket.gethostbyname(settings.REGISTRY))
        except:
            pass
       
        if settings.UPSTREAM_DATA_BLOCK != None:
            logger.info(f'Dependency Block: {settings.UPSTREAM_DATA_BLOCK}')
            self.connect(
                f'http://{settings.UPSTREAM_DATA_BLOCK}/api/v1/pipeline/register')
        elif settings.REGISTRY != None:
            logger.info(
                f'Registering edge with self to {settings.REGISTRY}: {settings.UPSTREAM_DATA_BLOCK}')
            host = socket.gethostbyname(socket.gethostname())
            self.connect(
                f'http://{settings.REGISTRY}/api/v1/pipeline/edge?upstream={settings.UPSTREAM_DATA_BLOCK}&downstream={host}')
    ...
    def get_dependency_url(self):
        return f'http://{settings.UPSTREAM_DATA_BLOCK}/api/'
